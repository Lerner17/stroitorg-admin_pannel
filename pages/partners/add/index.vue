      ...
      <v-flex xs12 sm12 md8>
        <v-card>
          <v-toolbar color="primary" dark>
            <v-toolbar-title>Добавить партнёра</v-toolbar-title>
          </v-toolbar>
          <v-card-text>
            <v-container grid-list-md>
              <v-form ref="form" v-model="valid">
                <v-layout wrap>
                  ...
                  <v-flex xs12>
                    <v-file-input name="logo" label="Логотип" :rules="[v => !!v || 'Поле не может быть пустым']" />
                  </v-flex>
                </v-layout>
...
export default {
  ...
  methods: {
    postModel () {
      ...
      this.$axios.$post('/partners/', formData).then(() => {
